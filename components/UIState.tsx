...
export default function UIState() {
  useEffect(() => {
    ...
    const loop = () => {
      ...
      const sceneOp = focused ? 1 : clamp(1 - (p - 0.53) / 0.09, 0.34, 1); // room stays a glowing backdrop, never a void
      root.style.setProperty("--scene-op", sceneOp.toFixed(3));
      root.style.setProperty("--prog", p.toFixed(4)); // the gold scroll hairline
      const phase = focused ? "focus" : p < 0.05 ? "intro" : p > 0.53 ? "outro" : "browse";
